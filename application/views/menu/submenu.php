                    ...
                    <?php foreach ($subMenu as $sm) : ?>
                        <tr>
                            ...
                            <td class="text-center">
                                <a href="" class="badge badge-warning" data-toggle="modal" data-target="#SubmenuEdit<?= $sm['id']; ?>">edit</a>
                                <a href="<?= base_url('menu/hapusSubmenu/') . $sm['id']; ?>" class="badge badge-danger" onclick="return confirm('Apakah Anda Yakin menghapus data tersebut ?');">delete</a>
                            </td>
                        </tr>
                        <?php $i++; ?>
                    <?php endforeach; ?>
...
<!-- Modal Edit -->
<?php foreach ($subMenu as $sm) : ?>
    <div class="modal fade" id="SubmenuEdit<?= $sm['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="SubmenuEditLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="SubmenuEditLabel">Ubah Submenu Baru</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="<?= base_url('menu/editSubmenu/') . $sm['id']; ?>" method="post">
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <input type="text" class="form-control" id="title" name="title" placeholder="Judul Submenu" value="<?= $sm['title']; ?>">
                        </div>
                        <div class="form-group">
                            <select name="menu_id" id="menu_id" class="form-control">
                                <option value="">Pilih Menu</option>
